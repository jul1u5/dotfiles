      ...
      lib.listToAttrs
        (map
          (ws:
            lib.nameValuePair
              (toString ws)
              (toString (if ws == 10 then 0 else ws)))
          (lib.range 1 10));
    "HDMI-A-1" =
      lib.listToAttrs
        (map
...
in
{
  ...
  home._ = home: {
    ...
    # Work-around: https://github.com/nix-community/home-manager/issues/2064#issuecomment-887300055
    ...
    wayland.windowManager.sway = {
      ...
      config =
        let
          cfg = home.config.wayland.windowManager.sway;
        ...
        in
        {
          ...
          output = {
            "*".bg = "${./faro16x9.jpg} fill";
          };

          seat.seat0 = {
            xcursor_theme = with home.config.xsession.pointerCursor;
              "${name} ${toString size}";
          };
          ...
          keybindings =
            let
              ...
              mkWorkspaceBinds = wsToKey: lib.mergeAttrs
                (lib.mapAttrs'
                  (ws: key:
                    lib.nameValuePair
                      "${mod}+${key}"
              ...
              workspaceBinds = lib.pipe outputToWorkspace [
                lib.attrValues
            ...
            in
            lib.mkOptionDefault ({
              "${mod}+Ctrl+${cfg.config.left}"  = "move container to output left; focus output left";
              "${mod}+Ctrl+${cfg.config.down}"  = "move container to output down; focus output down";
              "${mod}+Ctrl+${cfg.config.up}"    = "move container to output up; focus output up";
              "${mod}+Ctrl+${cfg.config.right}" = "move container to output right; focus output right";
          ...
          floating = {
            criteria = [
              ...
              { class = "^Vivaldi"; title = "^Vivaldi Settings"; }

              { class = "^jetbrains-studio$"; title = "^win0$"; }

              { class = "^Steam$"; title = "^Friends List$"; }
              { class = "^Steam$"; title = "^Steam Guard - Computer Authorization Required$"; }

              { app_id = "^org.gnome.(Settings|Software|Calculator)$"; }
          ...
          window = {
            ...
            commands = [
              ...
              {
                criteria = { app_id = "^firefox$"; title = "^Meet - .* - Mozilla Firefox$"; };
